...
;(function(window, document, fabric) {
  ...
  Darkroom.prototype = {
    ...
    initDOM: function(element) {
      ...
      // Save elements
      //! Searchable comment for Jamie - background for image overflow/letterboxing
      ...
      if (this.options.forceRatio != null) {
        height = Math.max(width, height) * this.options.forceRatio;
      }

      this.canvas.setWidth(this.options.maxWidth   || width);
      this.canvas.setHeight(this.options.maxHeight || height);
    ...
    createFabricImage: function(imgElement) {
      ...
      if (this.options.fillMode !== null) {
        if (this.options.fillMode !== 'contain') {
    ...
    // This creates a memory leak so commented out
